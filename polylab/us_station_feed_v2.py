...
def observation(station,timestamp,raw,temp,received,source):
    ...
    text=re.sub(r'^(?:METAR|SPECI)\s+','',(raw or '').strip())
    text=' '.join(text.split())
    if text and not text.startswith(station+' '):raise ValueError('Raw observation station mismatch')
    key=hashlib.sha256((station+'|'+str(int(timestamp))+'|'+text).encode()).hexdigest() if text else None
    observation_key=key or hashlib.sha256(json.dumps([source,station,timestamp,temp],sort_keys=True).encode()).hexdigest()
    return dict(station=station,observed_at=timestamp,raw_message=text,temperature=temp,available_at=received,source=source,report_key=key,observation_key=observation_key,raw_message_available=bool(text),
...
def paired_snapshot(observations,stations):
    ...
    for station in stations:
        ...
        result.append(dict(station=station,awc=a,nws=n,awc_newer_observation_seconds=a['observed_at']-n['observed_at'] if a and n else None,
                           same_report=bool(a and n and a['report_key'] and n['report_key'] and a['report_key']==n['report_key']),profit=None))
    return result
