"""Shared-account event-time simulation for intraday Benter comparisons."""
import bisect,heapq
from .us_accounting import Account,number,conservative_taker_fee as fee
from .us_benter_intraday import options
from .us_multiband import select
...
def simulate(groups, cfg, method, slip):
    ...
    while events:
        ...
        if kind == 'decision':
            city_options, mapping = {}, {}
            for g in payload:
                city_options[g['station']], reason = options(g, cfg, slip, method)
                mapping[g['station']] = g
                if reason: rejections.append(dict(t=t, station=g['station'], reason=reason))
            available = account.cash-account.reserve
            chosen = select(city_options, available, 'multi_band')
            decisions.append(dict(t=t, available=str(available), eligible_cities=sum(bool(o) for o in city_options.values()), selected=chosen))
            for o in chosen:
